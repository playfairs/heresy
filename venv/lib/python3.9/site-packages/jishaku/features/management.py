...
class ManagementFeature(Feature):
    ...
    @Feature.Command(parent="jsk", name="load", aliases=["reload", "refresh", "rl", "bbc"])
    ...
    @Feature.Command(parent="jsk", name="unload", aliases=["disable"])
    ...
    @Feature.Command(parent="jsk", name="shutdown", aliases=["logout", "spank", "kys", "die"])
    async def jsk_shutdown(self, ctx: ContextA):
        ...
        await ctx.send(f"oh ok :(")
        await ctx.bot.close()

    @Feature.Command(parent="jsk", name="invite", aliases=["scope"])
